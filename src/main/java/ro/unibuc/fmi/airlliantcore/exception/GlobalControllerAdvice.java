package ro.unibuc.fmi.airlliantcore.exception;
...
public class GlobalControllerAdvice {
    ...
    public ResponseEntity<List<ApiError>> handleAccessDeniedException(AccessDeniedException e) {

        log.warn("{} HTTP Status {}", e.getMessage(), HttpStatus.FORBIDDEN);

        return new ResponseEntity<>(
                Collections.singletonList(ApiError.builder()
                        .errorMessage(e.getMessage())
                        .errorCode(ExceptionStatus.ErrorCode.AIRLLIANT_FORBIDDEN)
                        .build()),
    ...
    public ResponseEntity<List<ApiError>> handleWebClientResponseException(WebClientResponseException e) {
        ...
        try {
            ...
            List<ApiError> receivedErrorList = Arrays.asList(mapper.readValue(e.getResponseBodyAsString(), ApiError[].class));
